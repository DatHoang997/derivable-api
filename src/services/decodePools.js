...
exports.parseDdlLogs = async (ddlLogs) => {
  console.log(ddlLogs)
...
exports.convertdata = async (pool) => {
  const result = []
  const [baseToken, quoteToken, token_r] = await getTokenSymbol(
    pool.baseToken,
    pool.quoteToken,
    pool.TOKEN_R,
  )
  ...
  const last_price = pool.states.twap.toString()
  const product_type = "RERP"
  const index_price = pool.states.twap.toString()
  const index_name = baseToken
  const index_currency = pool.states.twap.toString()
...
const getTokenSymbol = async (baseToken, quoteToken, TOKEN_R) => {
  ...
  console.log(results.results.baseToken.callsReturnContext[0].returnValues[0])
  return [
 ...
 */
//@ts-ignore
const loadStatesData = async (listTokens, listPools, uniPools) => {
  const multicall = new Multicall({
    multicallCustomContractAddress:
      "0xcA11bde05977b3631167028862bE2a173976CA11", //this.addresses.multiCall,
    ethersProvider: getPoolOverridedProvider(Object.keys(listPools)),
    tryAggregate: true,
  })
  const normalTokens = _.uniq(getNormalAddress(listTokens))

  // @ts-ignore
  const context = getMultiCallRequest(normalTokens, listPools)

  const { results } = await multicall.call(context)

  const pairsInfo = await UNIV3PAIR.getPairsInfo({
    pairAddresses: _.uniq(uniPools),
  })

  const { tokens: tokensArr, poolsState } = parseMultiCallResponse(
    results,
  ...
  for (const i in pools) {
    ...
    const rdc = getRdc(Object.values(poolGroups[id].pools))
    poolGroups[id].states = {
      ...poolGroups[id].states,
      ...rdc,
    }
...
const getRentRate = ({ rDcLong, rDcShort, R }) => {
...
const getPoolOverridedProvider = (poolAddresses) => {
  const stateOverride = {}
...
const getBasePrice = (pairInfo, baseToken) => {
 ...
 */
//@ts-ignore
const getMultiCallRequest = (normalTokens, listPools) => {
  const request = [
    {
      reference: "tokens",
...
const parseMultiCallResponse = (multiCallData, poolAddresses) => {
  const pools = {}
...
const parseDdlLogs = async (ddlLogs) => {
  const eventInterface = new ethers.utils.Interface(EventsAbi)
  const result = []
  for (const log of ddlLogs) {
    try {
...
const getLastBlock = async () => {
  ...
  return provider.getBlock(lastBlock)
}
